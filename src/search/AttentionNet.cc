...
void AttentionNet::initialize_ActionM_input_Helper(
    ...
    for (it = _FluentsSchema.begin(); it != _FluentsSchema.end(); it++) {
        ...
        if (it->patameters_types.size() == 0) {
            ...
            if (this->v_unique(it->patameters_types)) {
...
void AttentionNet::save_parameters(string path) {
    ...
    for (it2 = this->aggregation_attentions.begin();
        ...
        for (it1 = it2->second.begin(); it1 != it2->second.end(); it1++) {
            string mname =
                "aggregation_attentions+" + it2->first + "+" + it1->first + "+";
            it1->second.save_submodel(path, mname);
        }
    }
...
std::vector<double> AttentionNet::tensor2vector_z(torch::Tensor in) {
...
torch::Tensor AttentionNet::forward() {
    if (state_net->size()==0) {
        SystemUtils::abort("state_pointer_set not set! use set_state_pointer()");
    }
    ...
    cout << endl;
    return this->forward(current_state_);
}
...
bool AttentionNet::v_unique(const vector<string>& vec) {
    set<string> tmp;
