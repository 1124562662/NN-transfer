...
void RDDLTask::addCPF(ParametrizedVariable variable,
    ...
    taskFile << "The sf:"<< std::endl;
    variable.print(taskFile);
    taskFile << std::endl;

    for (int i=0;i<variable.params.size();++i){
         taskFile << "obj:"<< std::endl;
         variable.params[i]->print(taskFile);
         taskFile<< std::endl;// ??
         taskFile<< variable.params[i]->type->name<< std::endl;
    }
    ...
    //4550tmp
    // logicalExpression->print(std::cout);
    std::stringstream os_z;
    logicalExpression->print(os_z);
    std::string expression_z = os_z.str();
    int is_Braket1 =0;// ()
    int is_Braket2 =0;// {}
    int is_Braket3 =0;// []
    int contains_question_mark=0; // ?
    std::string last = "";  std::string last2 = "";
    for (std::string::size_type i = 0; i < expression_z.size(); ++i) {
    if(expression_z[i] == '{'){ is_Braket2 =1;
    } else if(expression_z[i] == '}'){ is_Braket2 =0;
    }else if(expression_z[i] == '['){ is_Braket3 =1;
    } else if(expression_z[i] == ']'){ is_Braket3 =0;
    }else if(is_Braket2 ==1 || is_Braket3 ==1 ){
    }else if(expression_z[i] == '('){
            is_Braket1 = 1;
            last2=last;
            last="";
    }else if(expression_z[i] == ')') {
          if (is_Braket1  == 1 && contains_question_mark ==1) {
                    //std::cout<< "obj s :"<<last<<std::endl;
                    taskFile <<""<<std::endl;
                    taskFile <<"fix-fluent_name:"<<std::endl;
        ...
        if (expression_z[i] == '?'){contains_question_mark =1;}
        last += expression_z[i];
         }
    }
    taskFile.close();
...
void RDDLTask::addObject(std::string const& typeName,
    ...
    //4550 tmp
    std::cout<< "object: "<< objectName<<" ,type name: "<<typeName <<std::endl;
    ...
    taskFile_z<<std::endl;
    taskFile_z<<"object-and-type:"; taskFile_z<<std::endl;
    taskFile_z<<objectName;         taskFile_z<<std::endl;
    taskFile_z<<typeName;           taskFile_z<<std::endl;
    taskFile_z.close();
    //4550 tmp
...
NonFluent* RDDLTask::getNonFluent(std::string const& name) {
   ...
   // 4550
    ...
    taskFile << std::to_string(nonFluentMap[name]->initialValue )<< std::endl;
    taskFile.close();
     // 4550
...
std::vector<StateFluent*> RDDLTask::getStateFluentsOfSchema(
    ...
    // for ( auto it = stateFluentsBySchema.begin(); it != stateFluentsBySchema.end(); ++it  )
    //        {
    // std::cout << "~~~~"<< std::endl;schema->print(std::cout);std::cout << "~~~~"<< std::endl;
    // for(int i=0; i<stateFluentsBySchema[it->first].size() ;++i){
