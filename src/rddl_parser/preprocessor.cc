...
void Preprocessor::prepareActions() {
    ...
    if (useIPC2018Rules) {
        ...
        while (true) {
            // cout << "Generating action states with " <<
            // task->numberOfConcurrentActions << " many action fluents." <<
            // endl; cout << "Total number of legal action states: " <<
            // legalActionStates.size() << endl; cout << "Number of base
            // actions: " << base.size() << endl;
            ...
            for (const ActionState& actionState : candidates) {
                ...
                for (unsigned int i = 0; i < task->staticSACs.size(); ++i) {
                    double res = 0.0;
                    task->staticSACs[i]->formula->evaluate(res, current,
                                                           actionState);
                    if (MathUtils::doubleIsEqual(res, 0.0)) {
                        isLegal = false;
                        break;
            ...
            if (!foundLegal || (task->numberOfConcurrentActions ==
                                task->actionFluents.size())) {
                break;
            }
            ++task->numberOfConcurrentActions;
...
void Preprocessor::calcAllActionStates(vector<ActionState>& result,
    ...
    if (result.empty()) {
        ...
        std::cout << std::to_string(task->actionFluents.size()) << "find_point_"
                  << std::to_string(lastIndex) << std::endl;
...
void Preprocessor::precomputeEvaluatable(Evaluatable* eval) {
    ...
    // cout << eval->name << endl;
    //     for (auto elem : dependentStateFluents)
    // {    elem->print(cout);cout<< ","<< endl;
    // }cout<< ".."<< endl;
