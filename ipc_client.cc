...
void IPCClient::run(string const& instanceName, string& plannerDesc) {
    ...
    // relation_net::trainnet_sogbofa();
    //   SystemUtils::abort("training ends");
    ...
    for (unsigned int currentRound = 0; currentRound < numberOfRounds;
        ...
        planner->rn.trainnet_sogbofa_ug(this->stateVariableIndices,
                                        this->action_indices, X);
        X.clear();
    }

    planner->rn.save_modules("E2/");
...
void IPCClient::initSession(string const& instanceName, string& plannerDesc) {
    ...
    relation_net rnn = relation_net(4, 2, 4, this->stateVariableIndices,
                                    this->action_indices, true);

    planner = std::unique_ptr<ProstPlanner>(new ProstPlanner(plannerDesc, rnn));
    planner->initSession(numberOfRounds, remainingTime);

    // 4550 tmp

    // SystemUtils::abort("end training");
